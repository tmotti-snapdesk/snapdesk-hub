...
export default function EspacesProposesPage() {
  ...
  return (
    <div className="max-w-6xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#1a3a5c]">Espaces proposés par Snapdesk</h1>
        <p className="text-slate-500 mt-1">
          Sélection établie pour votre recherche :{" "}
          <span className="font-medium text-[#1a3a5c]">{projet?.nom}</span>
        </p>
      </div>

      {/* Info banner */}
      <div className="flex items-start gap-3 p-4 bg-blue-50 rounded-xl border border-blue-100">
        <Info className="w-5 h-5 text-[#1a3a5c] flex-shrink-0 mt-0.5" />
          ...
          return (
            ...
            >
              ...
              <div className="h-40 bg-gradient-to-br from-slate-100 to-slate-200 rounded-t-xl flex items-center justify-center relative">
                ...
                {espace.score && (
                  <div className="absolute top-3 left-3 bg-[#1a3a5c] text-white text-xs font-bold px-2.5 py-1 rounded-full">
                    Score {espace.score}%
                  </div>
                )}
                <button
              ...
              <CardContent className="p-5 space-y-3">
                <div>
                  <h3 className="font-bold text-[#1a3a5c] leading-snug">{espace.nom}</h3>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="bg-slate-50 rounded-lg p-2">
                    ...
                    <p className="text-xs font-semibold text-[#1a3a5c]">{espace.superficie} m²</p>
                  </div>
                  <div className="bg-slate-50 rounded-lg p-2">
                    <Users className="w-3.5 h-3.5 text-slate-400 mx-auto mb-0.5" />
                    <p className="text-xs font-semibold text-[#1a3a5c]">{espace.capacite} postes</p>
                  </div>
                  <div className="bg-slate-50 rounded-lg p-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {espace.amenagements.slice(0, 3).map((a) => (
                    <span
                      key={a}
                      className="text-[10px] px-2 py-0.5 bg-blue-50 text-[#1a3a5c] rounded-full"
                    >
                      {a}
                    </span>
                  ))}
      ...
      {selection.length > 0 && !feedbackSent && (
        <Card className="border-2 border-amber-200 shadow-sm bg-amber-50">
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {espaces
                ...
                .map((e) => (
                  <span
                    key={e.id}
                    className="text-sm px-3 py-1.5 bg-white rounded-full border border-amber-200 text-amber-700 font-medium"
                  >
                    {e.nom}
                  </span>
                ))}
            ...
            <div className="space-y-2">
              ...
              <textarea
                className="w-full rounded-lg border border-amber-200 bg-white p-3 text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 resize-none"
                rows={3}
